...
# def make_laser_pose(aim_point, base_point):
#     # Vector từ robot đến điểm chiếu
#     x_axis = aim_point - base_point
#     x_axis = x_axis / np.linalg.norm(x_axis)

#     # Chọn z axis sao cho vuông với x (tùy chọn)
#     tmp = np.array([0, 0, 1]) if abs(x_axis[2]) < 0.9 else np.array([0, 1, 0])
...
#     return SE3(base_point) @ SE3.Rt(R, base_point)
...
with open(input_file, "r") as f:
    for line in f:
        ...
        # if x_offset is None and z_offset is None:
        #     x_offset = x_gcode
